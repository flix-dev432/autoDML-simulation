...
def autoDML(X, A, Y, n_splits=5, lambda_theta=0.001, lambda_alpha=0.001, theta_epochs=100, alpha_epochs=100, 
            batch_size=64, lr_theta=0.01, lr_alpha=0.01, stabilization=True, seed=None):
    """
    Implement the autoDML algorithm with cross-fitting
    ...
    for s in range(n_splits):
        ...
        theta_net = train_theta(theta_net, A_train, X_train, Y_train, 
    ...
    for s in range(n_splits):
        ...
        for epoch in range(alpha_epochs):
            ...
            for batch_local_idx, batch_a, batch_x in train_dataloader:
                ...
                for local_idx, a, x in zip(batch_local_idx, batch_a, batch_x):
                    # Get the original index of this sample
                    orig_idx = train_to_original[local_idx.item()]
                    
                    # Get the fold this sample belongs to (in the original dataset)
                    fold = fold_mapping[orig_idx]
                    
                    # Get theta model trained WITHOUT this sample's fold
                    # This is the crucial part: we use theta_{n,j(i)} which is the theta model
                    # trained without the fold containing sample i
                    theta_net = theta_models[fold]
                    theta_net.eval()
                    
                    # Calculate alpha loss using the corresponding theta model
                    with torch.no_grad():
                        # Get theta prediction
                        theta_ax = theta_net(a.unsqueeze(0), x.unsqueeze(0)).squeeze()
                        
                        # Calculate sigmoid derivative: sigmoid(x)*(1-sigmoid(x))
                        sigmoid_theta = torch.sigmoid(theta_ax)
                        sigmoid_deriv = sigmoid_theta * (1 - sigmoid_theta)
                    
                    # Alpha for current (a,x)
                    a_tensor = a.unsqueeze(0)
                    x_tensor = x.unsqueeze(0)
                    alpha_ax = alpha_net(a_tensor, x_tensor).squeeze()
                    
                    # Alpha differences (alpha(1,X) - alpha(0,X))
                    alpha_1x = alpha_net.g1(x_tensor).squeeze()
                    alpha_0x = alpha_net.g0(x_tensor).squeeze()
                    alpha_diff = alpha_1x - alpha_0x
                    
                    # Loss terms
                    first_term = sigmoid_deriv * alpha_ax**2
                    second_term = -2 * alpha_diff
                    
                    # Add to batch loss
                    sample_loss = first_term + second_term
    ...
    for batch in range(n_batches):
        ...
        for i in batch_indices:
            ...
            with torch.no_grad():
                ...
                batch_terms.append(term_i)
        
        # Add batch sum to total
        sum_term += sum(batch_terms)
    ...
    for batch in range(n_batches):
        ...
        for i in batch_indices:
            ...
            with torch.no_grad():
                ...
                batch_terms.append(squared_diff)
        
        # Add batch sum to total
        sum_squared_diff += sum(batch_terms)
...
def process_single_rep(rep, size, true_psi, n_splits, lambda_theta, lambda_alpha, stabilization):
    ...
    res = autoDML(X, A, Y, n_splits=n_splits, lambda_theta=lambda_theta, lambda_alpha=lambda_alpha,
                 theta_epochs=100, alpha_epochs=100, stabilization=stabilization, seed=seed)
...
def run_simulation(sample_sizes=[1000, 5000, 10000], n_reps=100, n_splits=5, 
    ...
    for size in sample_sizes:
        ...
        rep_results = Parallel(n_jobs=n_jobs)(
            delayed(process_single_rep)(rep, size, true_psi, n_splits, lambda_theta, lambda_alpha, stabilization)
...
if __name__ == "__main__":
    ...
    stabilization = True  # Apply TMLE-inspired stabilization
